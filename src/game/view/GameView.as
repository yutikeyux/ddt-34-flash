package game.view
{
   ...
   public class GameView extends GameViewBase
   {
	  ...
	  private function __mirariEffectShowHandler(param1:CrazyTankSocketEvent) : void
	  {
		  ...
		  var _loc4_:int = _loc2_.readInt();
		  var _loc5_:Boolean = _loc2_.readBoolean();
		  if(_loc4_ == MirariType.Targeting) 
		  {
			  if(_playerThumbnailLController && (_playerThumbnailLController as Object).hasOwnProperty("currentBoss"))
			  {
				  _loc6_ = (_playerThumbnailLController as Object).currentBoss as Living;
			  }
			  if(_loc6_ == null)
			  {
				  _loc6_ = _gameInfo.findLiving(_loc3_);
			  }
		  }
		  else
		  {
      ...
      private function __missionOver(param1:CrazyTankSocketEvent) : void
      {
         ...
         this._missionAgain.value = _gameInfo.missionInfo.TryAgainCost;
      ...
      private function showTryAgainCost() : void
      {
         var _loc1_:TryAgainCost = new TryAgainCost(this._missionAgain);
         _loc1_.addEventListener(GameEvent.TryAgainCost,this.__TryAgainCost);
         _loc1_.addEventListener(GameEvent.GIVEUP,this.__giveup);
         _loc1_.addEventListener(GameEvent.TIMEOUT,this.__TryAgainCostTimeOut);
         _loc1_.show();
         addChild(_loc1_);
      }
      
      private function canShowTryAgainCostByRoomType() : Boolean
      {
      ...
      private function __TryAgainCostTimeOut(param1:GameEvent) : void
      {
         param1.currentTarget.removeEventListener(GameEvent.TryAgainCost,this.__TryAgainCost);
         param1.currentTarget.removeEventListener(GameEvent.GIVEUP,this.__giveup);
         param1.currentTarget.removeEventListener(GameEvent.TIMEOUT,this.__TryAgainCostTimeOut);
      ...
      private function __expShowed(param1:GameEvent) : void
      {
         ...
         if((GameManager.isDungeonRoom(_gameInfo) || GameManager.isAcademyRoom(_gameInfo)) && _gameInfo.missionInfo.TryAgainCost > 0)
         {
            if(RoomManager.Instance.current.selfRoomPlayer.isViewer && !this._missionAgain.win && this.canShowTryAgainCostByRoomType())
            {
               this.showTryAgainCost();
               if(this._expView)
               {
            ...
            else if(!_gameInfo.selfGamePlayer.isWin && this.canShowTryAgainCostByRoomType())
            {
               this.showTryAgainCost();
               if(this._expView)
               {
      ...
      private function __giveup(param1:GameEvent) : void
      {
         ...
         {
            GameInSocketOut.sendMissionTryAgainCost(0,true);
         }
         this._expView.close();
         this._expView = null;
      }
      
      private function __TryAgainCost(param1:GameEvent) : void
      {
         param1.currentTarget.removeEventListener(GameEvent.TryAgainCost,this.__TryAgainCost);
         param1.currentTarget.removeEventListener(GameEvent.GIVEUP,this.__giveup);
         param1.currentTarget.removeEventListener(GameEvent.TIMEOUT,this.__TryAgainCostTimeOut);
