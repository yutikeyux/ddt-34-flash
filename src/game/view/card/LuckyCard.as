package game.view.card
{
	...
	public class LuckyCard extends Sprite implements Disposeable
	{
		...
		public function play(param1:Player, param2:int, param3:int, param4:Boolean) : void
		{
			...
			// --- Fake Item Mantığı ---
			// Eğer kartın sahibi henüz belirlenmediyse (param1 == null) ve sunucudan item düştüyse (param2 >= 0)
			if (param1 == null && param2 >= 0)
			{
				var pveId:int = 0;
				var isLastStage:Boolean = false;
				
				if(GameManager.Instance.Current && GameManager.Instance.Current.missionInfo)
				{
					pveId = GameManager.Instance.Current.missionInfo.id;
					isLastStage = (GameManager.Instance.Current.missionInfo.missionIndex == GameManager.Instance.Current.missionInfo.totalMissiton);
				}
				
				var fakeItems:Array = [];
				var fakeChance:Number = 0.01; // %1 ihtimalle fake item göster. (1.0 = %100, 0.1 = %10)
				
				if (isLastStage) 
				{
					if (pveId == 1072) { // Bogo Kolay
						fakeItems = [313105, 313106];
					} else if (pveId == 1173) { // Bogo Normal
						fakeItems = [17001, 17003];
					} else if (pveId == 1274) { // Bogo Zor
						fakeItems = [7035, 17004];
					} else if (pveId == 1375) { // Bogo Kahraman
						fakeItems = [7024, 17002];
					} else if (pveId == 2002) { // Karınca Kolay
						fakeItems = [];
					} else if (pveId == 2102) { // Karınca Normal
						fakeItems = [7015, 7019];
					} else if (pveId == 4103) { // Kale Normal
						fakeItems = [7018, 7023];
					} else if (pveId == 4203) { // Kale Zor
						fakeItems = [7037];
					} else if (pveId == 4303) { // Kale Kahraman
						fakeItems = [7027];
					} else if (pveId == 5104) { // Ejder Normal
						fakeItems = [7021, 7020];
					} else if (pveId == 5204) { // Ejder Zor
						fakeItems = [7087];
					} else if (pveId == 5304) { // Ejder Kahraman
						fakeItems = [7032];
					} else if (pveId == 6104) { // Olimpiyat Normal
						fakeItems = [7054];
					} else if (pveId == 6204) { // Olimpiyat Zor
						fakeItems = [7028];
					} else if (pveId == 6304) { // Olimpiyat Kahraman
						fakeItems = [7115, 17011];
					} else if (pveId == 7004) { // Civciv Kolay
						fakeItems = [7005];
					} else if (pveId == 7104) { // Civciv Normal
						fakeItems = [7040, 31001, 32006, 30001];
					} else if (pveId == 7204) { // Civciv Zor
						fakeItems = [7041, 17023];
					} else if (pveId == 12004) { // Anafor Kolay
						fakeItems = [];
					} else if (pveId == 12104) { // Anafor Normal
						fakeItems = [];
					} else if (pveId == 12204) { // Anafor Zor
						fakeItems = [7098];
					} else if (pveId == 12304) { // Anafor Kahraman
						fakeItems = [7145];
					} else if (pveId == 3106) { // Kabile Normal
						fakeItems = [7017, 7016];
					} else if (pveId == 3206) { // Kabile Zor
						fakeItems = [7036];
					} else if (pveId == 3306) { // Kabile Kahraman
						fakeItems = [7026];
					}
				}
				
				// Eğer listede item varsa ve şansımız yeterliyse fake itemi uygula
				if(fakeItems.length > 0 && Math.random() < fakeChance)
				{
					param2 = fakeItems[Math.floor(Math.random() * fakeItems.length)];
					param3 = 1;
				}
			}
			...
			finally
			{
				...
				if(this._templateID > 0)
				{
					...
					if(tempInfo && this._itemCell)
					{
						...
						// --- DÜZELTME: İsim ve Adet Yazdırma ---
						var itemNameStr:String = tempInfo.Name;
						if(this._count >= 1)
						{
							itemNameStr = itemNameStr + " x" + this._count.toString();
						}
						this._itemName.text = itemNameStr;
