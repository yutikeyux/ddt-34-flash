   ...
   public class EnergyView extends Sprite implements Disposeable
   {
	  ...
	  private function __enterFrame(param1:Event) : void //
	  {
		  if(KeyboardManager.isDown(Keyboard.SPACE))
		  {
			  ...
			  // Eğer FightLib (Eğitim) modundaysa ve açı vs. uygun değilse ateşlemeyi engelle.
			  // Not: Sadece 'isWork' kontrolü tüm eğitim modunu kilitleyebilir. 
			  // "Sabit açıda değil" hatası alınıyorsa, açının doğru olup olmadığını kontrol eden bir mantık eklenmelidir.
			  // Örneğin: if(FightLibManager.Instance.isWork && !AcikKontrolu)
			  if(FightLibManager.Instance.isWork)
			  {
				  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("tank.fightLib.AngleMessage"));
