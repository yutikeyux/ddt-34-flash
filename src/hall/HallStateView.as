package hall
{
	...
	public class HallStateView extends BaseStateView
	{
		...
		private var _dynamicBgBitmap:Bitmap;
		...
		override public function enter(param1:BaseStateView, param2:Object = null) : void
		{
			...
			if(!this._hallMainView)
			{
				...
				loader.addEventListener(IOErrorEvent.IO_ERROR, __onBgConfigError);
			...
			addChild(this.list);
			this.list.visible = false; //element
			this.initBuild();
		...
		private function defaultRightWonderfulPlayIconShow() : void
		{
			...
			// --- EKLENEN: WorldBoss Entrance 2 ---
			//HallIconManager.instance.updateSwitchHandler(HallIconType.WORLDBOSSENTRANCE2, true);
			// ------------------------------------
		}
		...
		{
			if(PathManager.AccumulativeEnable)
			{
			...
			// Yorum Satırlarında Kaldırılmış Etkinlikler
		...
		private function initBuild() : void
		{
			...
			this.setBuildState(6,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(5,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(11,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(2,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(7,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(1,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(0,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(9,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(10,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(4,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
			this.setBuildState(8,_loc1_ >= 1 ? Boolean(Boolean(true)) : Boolean(Boolean(false)));
		}
		...
		private function setFarmIcon() : void
		{
			...
			this._farmIcon.visible = false; //element
		}
		...
		// ==============================================
		// YENİ EKLENEN ARKA PLAN YÖNETİM FONKSİYONLARI
		// ==============================================
		private function __onBgConfigLoaded(e:Event) : void
		{
			var loader:URLLoader = e.target as URLLoader;
			loader.removeEventListener(Event.COMPLETE, __onBgConfigLoaded);
			loader.removeEventListener(IOErrorEvent.IO_ERROR, __onBgConfigError);
			
			var bgType:String = String(loader.data).replace(/\s/g, ""); 
			var bgName:String = "asset.hall.hallBG" + bgType;
			
			if(bgType == "" || bgType == "Default" || bgType == "Gunduz") 
			{
				return; 
			}
		...
		private function __onBgConfigError(e:IOErrorEvent) : void
		{
			var loader:URLLoader = e.target as URLLoader;
			loader.removeEventListener(Event.COMPLETE, __onBgConfigLoaded);
			loader.removeEventListener(IOErrorEvent.IO_ERROR, __onBgConfigError);
		...
		private function applyBackground(bgName:String) : void
		{
			try {
				var BgClass:Class = flash.utils.getDefinitionByName(bgName) as Class;
				if(BgClass != null) {
					if(_dynamicBgBitmap && _dynamicBgBitmap.parent) {
						_dynamicBgBitmap.parent.removeChild(_dynamicBgBitmap);
					}
					var bmd:* = new BgClass(0,0);
					_dynamicBgBitmap = new flash.display.Bitmap(bmd);
					this._hallMainView.addChildAt(_dynamicBgBitmap, 1); 
					
					var isNight:Boolean = (bgName.indexOf("Aksam") != -1 || bgName.indexOf("Gece") != -1 || bgName.indexOf("SuAlti") != -1 || bgName.indexOf("Hell") != -1 || bgName.indexOf("CyberPunk") != -1 || bgName.indexOf("Matrix") != -1);
					
					var gunesObj:DisplayObject = this._hallMainView.getChildByName("gunesefekti");
					
					if(gunesObj != null) {
						if(isNight) {
							gunesObj.visible = false;
						} else {
							gunesObj.visible = true;
						}
					}
				}
			} catch(err:Error) {
