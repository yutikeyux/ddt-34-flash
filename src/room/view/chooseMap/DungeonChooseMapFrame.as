package room.view.chooseMap
{
	...
	public class DungeonChooseMapFrame extends Frame
	{
		...
		// VIP İndirim Oranını Hesaplayan Yeni Fonksiyon
		private function getVipDiscount() : int
		{
			var vipLevel:int = PlayerManager.Instance.Self.VIPLevel;
			switch(vipLevel)
			{
				case 2: return 5;
				case 3: return 10;
		...
		private function showAlert() : void
		{
			if(this._alert == null)
			{
				var price:String = this.getPrice();
				var discount:int = this.getVipDiscount();
				
				// Orijinal dil dosyasındaki mesajı alıyoruz
				var message:String = LanguageMgr.GetTranslation("tank.room.openBossTip.text", price);
				
				// Eğer indirim varsa (discount > 0), mesaja VIP bilgisini ekliyoruz
				if(discount > 0)
				{
					var vipLevel:int = PlayerManager.Instance.Self.VIPLevel;
					message = message + "\n\n" + "VIP seviyeniz " + vipLevel + " olduğu için %" + discount + " indirim uygulanacaktır.";
				}
				
				this._alert = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"), message, LanguageMgr.GetTranslation("ok"), LanguageMgr.GetTranslation("cancel"), false, false, false, LayerManager.BLCAK_BLOCKGOUND);
