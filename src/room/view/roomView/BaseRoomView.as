package room.view.roomView
{
	...
	import flash.display.Graphics; // Çizim için eklendi
	import flash.text.TextField; // Yazı için eklendi
	import flash.text.TextFormat; // Yazı formatı için eklendi
	import flash.text.TextFormatAlign; // Yazı hizalama için eklendi
	...
	// MusicFrame dosyasının konumuna göre import:
	import room.view.roomView.MusicFrame; 
	// Eğer MusicFrame.as dosyasını "room/view/roomView" klasörüne koyduysanız üsteki silip bunu açın:
	// import room.view.roomView.MusicFrame;
	...
	public class BaseRoomView extends Sprite implements Disposeable
	{
		...
		protected var _musicBtn:Sprite; // TextButton yerine Sprite kullandık (içine çizim yapacağız)
		...
		protected function initView() : void
		{
			...
			// --- YENİ BUTON OLUŞTURMA BAŞLANGIÇ ---
			// Butonu manuel olarak oluşturuyoruz (Harici dosya gerektirmez)
			this._musicBtn = new Sprite();
			
			// 1. Arka Plan Çizimi
			var g:Graphics = this._musicBtn.graphics;
			g.beginFill(0x1a1a1a, 0.8); // Koyu gri, biraz şeffaf arka plan
			g.lineStyle(1, 0xFFCC00);   // Sarı kenarlık
			g.drawRoundRect(0, 0, 70, 25, 8, 8); // Genişlik:70, Yükseklik:25, Köşe Yuvarlaklığı:8
			g.endFill();
			...
			tf.color = 0xFFFFFF; // Beyaz yazı
			tf.size = 14;
			tf.bold = true;
			tf.align = "center"; // Ortala
			
			txt.defaultTextFormat = tf;
			txt.setTextFormat(tf);
			
			this._musicBtn.addChild(txt);
			
			// 3. Konumlandırma (Invite butonunun solu)
			this._musicBtn.x = this._inviteBtn.x - 80; 
			this._musicBtn.y = this._inviteBtn.y;
			
			// 4. Sahneye Ekleme (EN SONA ekliyoruz ki üstte kalsın)
			addChild(this._musicBtn);
			
			this._musicBtn.buttonMode = true;
			// --- YENİ BUTON OLUŞTURMA BİTİŞ ---
		...
		protected function updateButtons() : void
		{
			...
			// Müzik butonu oyun başladığında solsun ama kapanmasın (veya isterseniz visible = false yapabilirsiniz)
			if(this._musicBtn)
			{
				this._musicBtn.alpha = !this._info.started ? 1 : 0.5;
				this._musicBtn.mouseEnabled = !this._info.started;
			}
		...
		protected function initTileList() : void
		{
			...
			if(this.isViewerRoom)
			{
				...
				while(_loc1_ < 10)
				{
					...
					{
						_loc2_ = new RoomViewerItem(_loc1_,RoomViewerItem.LONG);
					}
					else 
					{
		...
		protected function initEvents() : void
		{
			...
			// YENİ EKLEME: Müzik butonu olayı
			if(this._musicBtn)
			{
				this._musicBtn.addEventListener(MouseEvent.CLICK, this.__musicClick);
			}
		...
		// YENİ EKLEME: Müzik Butonu Tıklama Fonksiyonu
		protected function __musicClick(param1:MouseEvent) : void
		{
			SoundManager.instance.play("008");
			try 
			{
				var frame:MusicFrame = new MusicFrame();
				LayerManager.Instance.addToLayer(frame, LayerManager.GAME_BASE_LAYER);
			}
			catch(e:Error)
			{
				MessageTipManager.getInstance().show("MusicFrame yüklenemedi. Dosya konumunu kontrol edin.");
			}
		}
		...
		protected function removeEvents() : void
		{
			...
			// YENİ EKLEME: Müzik butonu olayını kaldır
			if(this._musicBtn)
			{
				this._musicBtn.removeEventListener(MouseEvent.CLICK, this.__musicClick);
			}
		...
		public function dispose() : void
		{
			...
			// YENİ EKLEME: Müzik butonunu temizle
			if(this._musicBtn)
			{
				// Sprite olduğu için ObjectUtils.disposeObject kullanılabilir veya manuel temizlik
				if(this._musicBtn.parent) this._musicBtn.parent.removeChild(this._musicBtn);
				this._musicBtn = null;
			}
