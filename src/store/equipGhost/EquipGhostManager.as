package store.equipGhost
{
   ...
   public class EquipGhostManager extends EventDispatcher
   {
      ...
      {
         SocketManager.Instance.addEventListener(PkgEvent.format(ePackageType.EQUIP_GHOST),this.__equipGhost);
         SocketManager.Instance.addEventListener(PkgEvent.format(ePackageType.SYNC_EQUIP_GHOST),this.__syncEquipGhost);
      }
      ...
      private function __syncEquipGhost(param1:PkgEvent) : void
      {
         ...
         while(_loc2_ < _loc4_)
         {
			...
			_loc5_ = new EquipGhostData(_loc6_,_loc7_)
			_loc5_.level = _loc3_.readInt();
