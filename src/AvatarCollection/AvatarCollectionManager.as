	...
	public class AvatarCollectionManager extends EventDispatcher
	{
		...
		private function getAllInfoHandler(param1:PackageIn) : void
		{
			...
			while(_loc10_ < _loc9_)
			{
				...
				if(_loc3_ == 1)
				{
					_loc4_ = this._maleUnitDic[_loc2_];
					_loc5_ = this._maleItemDic[_loc2_];
				}
				else
				{
					_loc4_ = this._femaleUnitDic[_loc2_];
					_loc5_ = this._femaleItemDic[_loc2_];
				}
				...
				while(_loc7_ < _loc6_)
				{
					...
					if(Boolean(_loc5_[_loc8_]))
					{
						(_loc5_[_loc8_] as AvatarCollectionItemVo).isActivity = true;
					}
					_loc7_++;
				}
				_loc4_.endTime = param1.readDate();
				_loc10_++;
			}
		}
