package setting.view
{
	...
	public class SettingView extends BaseAlerFrame
	{
		...
		// sağ alttaki "Varsayılanlar" butonu
		private var _btnReset:BaseButton;
		
		// yüzde yazıları
		private var _musicPercentTxt:TextField;
		private var _soundPercentTxt:TextField;
		...
		override public function dispose() : void
		{
			...
			var focusDO:DisplayObject = StageReferance.stage.focus as DisplayObject;
			if(focusDO && contains(focusDO))
			{
			...
			ObjectUtils.disposeObject(this._musicPercentTxt);
			this._musicPercentTxt = null;
			
			ObjectUtils.disposeObject(this._soundPercentTxt);
			this._soundPercentTxt = null;
		...
		private function initView() : void
		{
			...
			var infoObj:AlertInfo = new AlertInfo(LanguageMgr.GetTranslation("tank.game.ToolStripView.set"));
			infoObj.moveEnable = false;
			info = infoObj;
			
			// ESC ile kapanabilsin
			escEnable = true;
			...
			var p1:Point = ComponentFactory.Instance.creatCustomObject("setting.sliderBg1");
			p2 = ComponentFactory.Instance.creatCustomObject("setting.sliderBg2");
			
			this._sliderBg1 = ComponentFactory.Instance.creatBitmap("asset.setting.sliderBg");
			this._sliderBg1.x = p1.x;
			this._sliderBg1.y = p1.y;
			addToContent(this._sliderBg1);
			
			this._sliderBg2 = ComponentFactory.Instance.creatBitmap("asset.setting.sliderBg");
			this._sliderBg2.x = p2.x;
			this._sliderBg2.y = p2.y;
			...
			// yüzde text’leri
			this.createPercentTexts();
			
			// reset butonu
			this._btnReset = ComponentFactory.Instance.creatComponentByStylename("setting.resetButton");
			this._btnReset.addEventListener(MouseEvent.CLICK, this.__resetClick);
			addToContent(this._btnReset);
			
			this.initData();
		}
		
		private function createPercentTexts():void
		{
			var fmt:TextFormat = new TextFormat("Arial", 11, 0xFFFFFF, true);
			
			this._musicPercentTxt = new TextField();
			this._musicPercentTxt.width = 40;
			this._musicPercentTxt.height = 20;
			this._musicPercentTxt.mouseEnabled = false;
			this._musicPercentTxt.selectable = false;
			this._musicPercentTxt.defaultTextFormat = fmt;
			this._musicPercentTxt.text = "0%";
			this._musicPercentTxt.x = this._sliderBg1.x + this._sliderBg1.width - 45;
			this._musicPercentTxt.y = this._sliderBg1.y - 2;
			addToContent(this._musicPercentTxt);
			
			this._soundPercentTxt = new TextField();
			this._soundPercentTxt.width = 40;
			this._soundPercentTxt.height = 20;
			this._soundPercentTxt.mouseEnabled = false;
			this._soundPercentTxt.selectable = false;
			this._soundPercentTxt.defaultTextFormat = fmt;
			this._soundPercentTxt.text = "0%";
			this._soundPercentTxt.x = this._sliderBg2.x + this._sliderBg2.width - 45;
			this._soundPercentTxt.y = this._sliderBg2.y - 2;
			addToContent(this._soundPercentTxt);
		}
		
		private function updateMusicPercentLabel():void
		{
			if(this._musicPercentTxt)
				this._musicPercentTxt.text = this.musicVolumn.toString() + "%";
		}
		
		private function updateSoundPercentLabel():void
		{
			if(this._soundPercentTxt)
				this._soundPercentTxt.text = this.soundVolumn.toString() + "%";
		}
		...
		private function initData() : void
		{
			...
			// slider değerleri güncellenince % textlerini de güncelle
			this.updateMusicPercentLabel();
			this.updateSoundPercentLabel();
		}
		...
		// Varsayılanlar butonu – revert + UI yenile
		...
		private function sliderEnable(param1:Silder, param2:Bitmap, param3:Boolean) : void
		{
			...
			{
				filtersArr = [ComponentFactory.Instance.model.getSet("grayFilter")];
				param2.filters = filtersArr;
				param1.filters = filtersArr;
			}
		}
