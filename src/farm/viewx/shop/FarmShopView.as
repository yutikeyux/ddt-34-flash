package farm.viewx.shop
{
   ...
   public class FarmShopView extends Frame
   {
      ...
      private function __pageBtnClick(param1:MouseEvent) : void
      {
         ...
         switch(param1.currentTarget)
         {
            ...
            case this._prePageBtn:
               if(CURRENT_PAGE == 1)
               {
                  CURRENT_PAGE = ShopManager.Instance.getResultPages(this.getType(),8) + 1;
               }
               --CURRENT_PAGE;
               break;
            case this._nextPageBtn:
               if(CURRENT_PAGE == ShopManager.Instance.getResultPages(this.getType(),8))
               {
                  CURRENT_PAGE = 0;
               }
               ++CURRENT_PAGE;
            ...
            case this._endPageBtn:
               if(CURRENT_PAGE != ShopManager.Instance.getResultPages(this.getType(),8))
               {
                  CURRENT_PAGE = ShopManager.Instance.getResultPages(this.getType(),8);
               }
         }
         this.loadList();
      }
      ...
      {
		  this.setList(ShopManager.Instance.getValidSortedGoodsByType(this.getType(),CURRENT_PAGE,SHOP_ITEM_NUM));
      }
      ...
      public function setList(param1:Vector.<ShopItemInfo>) : void
      {
         ...
         this._currentPageTxt.text = CURRENT_PAGE + "/" + ShopManager.Instance.getResultPages(this.getType(),8);
      }
