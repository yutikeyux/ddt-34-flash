package ddt.view.chat
{
	...
	public class ChatFormats
	{
		...
		public static const BIG_BUGGLE_TYPE_STRING:Array = [
			"Çocuksu Aşk ",
		...
		public static function creatBracketsTag(param1:String, param2:int, param3:Array = null, param4:ChatData = null) : String
		{
			...
			// Stability: Null check for matches
			if(_loc6_ == null)
			{
				return param1;
			}
			...
			while(_loc8_ < _loc6_.length)
			{
				...
				// CHANGE: Removed the restriction that prevented the player from clicking their own name.
				// Previously: if(param2 != CLICK_USERNAME || _loc9_ != PlayerManager.Instance.Self.NickName)
				// Now: Everyone gets a link, including the player themselves.
		...
		public static function creatGoodTag(param1:String, param2:int, param3:int, param4:int, param5:Boolean, param6:ChatData = null, param7:String = "", param8:int = -1) : String
		{
			...
			// Stability: Safe access to zoneID
			var _loc12_:int = 0;
			if(param6 != null)
			{
				_loc12_ = param6.zoneID;
			}
			
			var _loc13_:int = 0;
			
			if(_loc11_ == null) return param1;
		...
		{
			if(param1 >= 0 && param1 < CHAT_COLORS.length)
			{
		...
		public static function getColorByBigBuggleType(param1:int) : int
		{
			...
			if(param1 >= 0 && param1 < BIG_BUGGLE_COLOR.length)
			{
		...
		{
			// Stability: Bounds check (Array is 0-based, param1 seems to be 1-based in usage)
			var index:int = param1 - 1;
			if(index >= 0 && index < BIG_BUGGLE_TYPE_STRING.length)
			{
				return BIG_BUGGLE_TYPE_STRING[index];
			}
			return "";
		}
		...
		private static function creatContentTag(param1:ChatData) : String
		{
			...
			if(param1.link)
			{
				...
				for each(_loc4_ in param1.link)
				{
					...
					if(_loc7_ == null) continue; // Stability: Null check for item template
		...
		private static function creatSenderTag(param1:ChatData) : String
		{
			...
			if(param1.channel == ChatInputView.PRIVATE)
			{
				...
				{
					// CHANGE: "receiver" is now also clickable via creatBracketsTag logic if applicable, 
					// but here the sender is sending to receiver.
				...
				{
					// CHANGE: The sender's name will now be clickable by the receiver (Self).
					// Since we removed the restriction in creatBracketsTag, this will generate a link.
			...
			{
				// CHANGE: Sender name becomes clickable link.
		...
		private static function setupStyle() : void
		{
			...
			var _loc2_:int = 0;
			// FIX: Changed <= to < to prevent Array Index Out Of Bounds error.
