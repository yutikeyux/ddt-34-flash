	...
	public class ChatOutputView extends Sprite
	{
		...
		public function set enableGameState(param1:Boolean) : void
		{
			if(param1)
			{
				...
				// ✅ scroll: default kapalı (MORE açınca açacağız)
				this.attachScrollToContainer(this._leftBtnContainerInGame);
			...
			else
			{
				...
				// ✅ scroll lobby’de left container’da dursun ama normalde görünmesin (lessen’da kapalı)
				this.attachScrollToContainer(this._leftBtnContainer);
				this._leftScroll.visible = this._more.selected; // lobby’de MORE varsa açılır, yoksa kapalı
		...
		public function set bg(param1:int) : void
		{
			...
			if(this.isInGame())
			{
				...
				// ✅ game’de scroll index/len güncelle
		...
		private function initBtns() : void
		{
			...
			var p1:Point = ComponentFactory.Instance.creatCustomObject("chat.OriginPos");
			var p2:Point = ComponentFactory.Instance.creatCustomObject("chat.PrivateBtnEffectPos");
			...
			this._privateBtnEffect = EffectManager.Instance.creatEffect(EffectTypes.ADD_MOVIE_EFFECT,this._privateBtn,"asset.chat.PrivateBtn_01","chat.ChatPrivateShineEffect",p1,p2);
		...
		private function attachScrollToContainer(container:Sprite):void
		{
			if(!this._leftScroll) return;
			if(this._leftScroll.parent && this._leftScroll.parent != container)
			{
				this._leftScroll.parent.removeChild(this._leftScroll);
			}
			if(container && !container.contains(this._leftScroll))
			{
				container.addChild(this._leftScroll);
			}
		}
		...
		{
			var inGame:Boolean = this.isInGame();
			
			// scroll hangi container’da olacak?
			this.attachScrollToContainer(inGame ? this._leftBtnContainerInGame : this._leftBtnContainer);
		...
		public function updateCurrnetChannel() : void
		{
			...
			if(this.isInGame())
			{
				// game’de senin gösterdiğin: 5
				this._leftScroll.rowsOfScreen = 5;
			}
			else
			{
				// lobby: less 6, more 16
				this._leftScroll.rowsOfScreen = this._more.selected ? 16 : 6;
			}
			
			if(this.isInGame())
			{
				obj = this._model.getChatsByOutputChannel(this._channel,this._currentOffset,5);
				
				// scroll bilgisi güncelle (MORE açıkken görünür)
				this._leftScroll.length = this._model.getRowsByOutputChangel(this._channel);
				this._leftScroll.currentIndex = this._currentOffset;
			}
			else
			{
				obj = this._model.getChatsByOutputChannel(this._channel,this._currentOffset,!!this._more.selected ? 16 : 6);
				this._leftScroll.length = this._model.getRowsByOutputChangel(this._channel);
				this._leftScroll.currentIndex = this._currentOffset;
			}
			
			this._currentOffset = obj["offset"];
			this._outputField.setChats(obj["result"]);
			this.goBottom();
			this.updateShine();
