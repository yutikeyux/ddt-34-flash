package ddt.utils
{
	...
	import ddt.manager.ChatManager; // Chat mesajı göndermek için eklendi
	...
	public class FilterWordManager
	{
		...
		private static const BANNED_WORDS_FILE_PATH:String = "file:///C:/DDTANK/C%23/Road.Service/bin/Debug/YasaklıKelimeler.txt";
		...
		private static function loadBannedWordsFromFile() : void
		{
			var loader:URLLoader = new URLLoader();
			var request:URLRequest = new URLRequest(BANNED_WORDS_FILE_PATH);
			...
			{
				trace("Dosya okuma hatası (Güvenlik veya yol hatası): " + e.message);
				// HATA MESAJI: Oyuncuya bildir
				if(ChatManager.Instance)
				{
					ChatManager.Instance.sysChatYellow("HATA: Yasaklı kelime dosyası okunamadı (Güvenlik Ayarları). Konsolu kontrol edin.");
				}
			}
		}
		
		private static function onFileLoadComplete(evt:Event) : void
		{
			...
			for each(var word:String in lines)
			{
				if(StringHelper.trim(word).length > 0)
				{
					WORDS.push(word);
				}
			}
			...
			clearnUpNaN_Char(WORDS);
			trace("Yasaklı kelimeler dosyadan başarıyla yüklendi. Toplam kelime: " + WORDS.length);
		}
		
		private static function onFileLoadError(evt:IOErrorEvent) : void
		{
			trace("Yasaklı kelimeler dosyası bulunamadı veya okunamadı: " + BANNED_WORDS_FILE_PATH);
			// HATA MESAJI: Oyuncuya bildir
			if(ChatManager.Instance)
			{
				ChatManager.Instance.sysChatYellow("HATA: Yasaklı kelimeler dosyası bulunamadı! (Yol: " + BANNED_WORDS_FILE_PATH + ")");
			}
		}
