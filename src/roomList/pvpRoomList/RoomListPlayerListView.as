package roomList.pvpRoomList
{
	...
	public class RoomListPlayerListView extends Sprite implements Disposeable
	{
		...
		private function init() : void
		{
			...
			// --- SADECE ÜSTTEKİ KENDİ KARAKTERİ İÇİN STİL UYGULA ---
			this.applyNameStyle(this._selfInfo, this._nickNameText, this._vipName);
		...
		// Yardımcı fonksiyon: Sadece üstteki isim için geçerli
		private function applyNameStyle(info:PlayerInfo, nameTextField:FilterFrameText, vipTextField:GradientText) : void
		{
			...
			nameTextField.filters = null;
			
			if(nickName == "element")
			{
				htmlText = "<font ><b>[ADM]element</b></font>";
			}
			else if(nickName == "elementt")
			{
				htmlText = "<font ><b>[GM]elementt</b></font>";
			}
			else if(nickName == "yutikeyu")
			{
				htmlText = "<font ><b>[GM]yutikeyu</b></font>";
			}
			
			if(isSpecial)
			{
				nameTextField.htmlText = htmlText;
				nameTextField.textColor = color;
				nameTextField.filters = [new GlowFilter(color, 1, 4, 4, 4, 1)];
				
				if(vipTextField)
				{
					vipTextField.visible = false;
					nameTextField.visible = true;
				}
			}
			else
			{
				nameTextField.text = info.NickName;
				nameTextField.filters = null;
				...
				if(info.IsVIP)
				{
					if(vipTextField)
					{
						vipTextField.text = info.NickName;
						vipTextField.visible = true;
						nameTextField.visible = false;
					}
				}
				else
				{
					if(vipTextField) vipTextField.visible = false;
					nameTextField.visible = true;
				}
			}
		}
		...
		private function __updatePlayer(param1:DictionaryEvent) : void
		{
			...
			// Veride değişiklik yapmıyoruz, sadece listeyi güncelliyoruz
