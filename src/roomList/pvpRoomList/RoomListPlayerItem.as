package roomList.pvpRoomList
{
	...
	public class RoomListPlayerItem extends Sprite implements Disposeable, IListCell
	{
		...
		{
			var nickName:String = this._info.NickName;
			var isSpecial:Boolean = false;
			
			// Önceki hücreden kalma efektleri temizle
			this._name.filters = null;
			this._name.textColor = 0xFFFFFF; // Varsayılan rengi beyaz yap (arkaplan koyuysa)
			
			// --- ÖZEL OYUNCU KONTROLÜ ---
			if(nickName == "element")
			{
				var modPart:String = "<font color=\"#FF0000\">[</font>" + 
					"<font color=\"#FFA500\">G</font>" + 
			...
			// --- NORMAL OYUNCU MANTIĞI ---
			if(!isSpecial)
			{
				// Normal ismi geri yükle
				this._name.text = nickName;
				
				if(this._info.IsVIP)
				{
					// VIP ise Sarı Gradient Text'i göster
					this._vipName.text = nickName;
					this._vipName.visible = true;
					this._name.visible = false;
				}
				else
				{
					// VIP değilse Normal Text'i göster
					this._name.visible = true;
					this._vipName.visible = false;
					// Rengi varsayılana (beyaz) çevirdik zaten başta
				}
			}
