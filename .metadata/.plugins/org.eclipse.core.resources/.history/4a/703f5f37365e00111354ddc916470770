package pyramid.view
{
   ...
   public class PyramidCards extends Sprite implements Disposeable
   {
      ...
      {
         _topBox.addEventListener(MouseEvent.CLICK,__cardClickHandler);
         _shuffleWaitTimer.addEventListener(TimerEvent.TIMER,__shuffleWaitTimerHandler);
      }
      ...
      {
         var _loc4_:PyramidCard;
         (_loc4_ = new PyramidCard()).index = param1 + "_" + param2;
         var _loc3_:Point = PositionUtils.creatPoint("pyramid.view.cardPos" + _loc4_.index);
         _loc4_.x = _loc3_.x;
         _loc4_.y = _loc3_.y;
         _cards[param1][param2] = _loc4_;
         _loc4_.addEventListener(MouseEvent.CLICK,__cardClickHandler);
         _loc4_.addEventListener(PyramidEvent.OPENANDCLOSE_MOVIE,__cardOpenMovieHandler);
      ...
      public function updateSelectItems() : void
      {
         ...
         {
            if(!_cardsSprite.hasEventListener(Event.ENTER_FRAME))
            {
               _cardsSprite.addEventListener(Event.ENTER_FRAME, __delayReset);
            }
         }
         else
         {
      ...
      private function __delayReset(param1:Event) : void
      {
         ...
         {
            reset();
            _cardsSprite.removeEventListener(Event.ENTER_FRAME, __delayReset);
         }
      }
      ...
      public function dispose() : void
      {
         ...
         PyramidControl.instance.isAutoOpenCard = false;
         _topBox.removeEventListener(MouseEvent.CLICK,__cardClickHandler);
         _shuffleWaitTimer.stop();
         _shuffleWaitTimer.removeEventListener(TimerEvent.TIMER,__shuffleWaitTimerHandler);
         _shuffleWaitTimer = null;
         for each(var _loc1_:* in _cards)
         {
            for each(var _loc2_:* in _loc1_)
            {
               _loc2_.removeEventListener(MouseEvent.CLICK,__cardClickHandler);
               _loc2_.removeEventListener(PyramidEvent.OPENANDCLOSE_MOVIE,__cardOpenMovieHandler);
               _loc2_.dispose();
            }
         }
         _cards = null;
